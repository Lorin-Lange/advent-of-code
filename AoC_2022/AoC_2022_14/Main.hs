...
check :: ((Int, Int) -> Grid -> Int -> Int) -> (Int, Int) -> Grid -> Int -> Int
...
check1 :: Int -> (Int, Int) -> Grid -> Int -> Int
check1 max k@(c, r) g counter
    | r > max   = counter
    | otherwise = check (check1 max) k g counter
...
main = do
    ...
    putStrLn $ "Part 1: " ++ show (check1 maxR (500, -1) m 0)
